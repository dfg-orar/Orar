 ...
 *
 */
public class TransitivityRuleExecutor2 implements RuleExecutor {
	/*
	 * store newly derived role assertions, which will be available in the todo
	 * of the ruleset, e.g. input for subrole-rule
	 */
	private final Set<IndexedRoleAssertion> newRoleAssertions;
	private boolean isIncrementalStepAfterFirstAbstraction = false;
	/*
	 * store saturated transitive role assertions
	 */
	private final Set<IndexedRoleAssertion> tranRoleSaturation;
	...
	private final Logger logger = Logger.getLogger(TransitivityRuleExecutor2.class);

	public TransitivityRuleExecutor2(OrarOntology2 orarOntology) {
		this.orarOntology = orarOntology;
		this.newRoleAssertions = new HashSet<IndexedRoleAssertion>();
		this.tranRoleSaturation = new HashSet<>();
