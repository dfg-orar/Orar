package orar.refinement.assertiontransferring.HornSHOIF_Increment;
...
public class HornSHOIF_AssertionTransporterOptimized_Increment extends AssertionTransporterTemplate_Increment {
	private Logger logger = Logger.getLogger(HornSHOIF_AssertionTransporterOptimized_Increment.class);
	private final IndividualIndexer indexer;
	private MetaDataOfOntology metaDataOfOntology;

	public HornSHOIF_AssertionTransporterOptimized_Increment(OrarOntology2 orarOntoloy,
	...
	protected void transferSameasAssertions() {
		...
		while (iterator.hasNext()) {
			...
			if (equivalentOriginalInds.size() > 1) {
				Integer firstIndiv = equivalentOriginalInds.iterator().next();
...
//					logger.info("indiv:"+firstIndiv);
//					logger.info("OldType:"+oldType);
					this.individualsHavingNewTypes.addAll(equivalentOriginalInds);
					// TODO: not moving them from old maps does not hurt but
					// could be improved.
					...
					if (this.config.getDebuglevels().contains(DebugLevel.TRANSFER_SAMEAS)) {
						...
						PrintingHelper
								.printSet(IndividualIndexer.getInstance().getOWLIndividuals(equivalentOriginalInds));
	...
	private void transferRoleAssertionBetweenUAndXAbstract() {
		...
		for (int index = 0; index < size; index++) {
			...
			Set<Integer> originalSubjects = new HashSet<>(
					this.dataForTransferingEntailments.getOriginalIndividuals(abstractSubject));
			Set<Integer> originalObjects = new HashSet<>(
					this.dataForTransferingEntailments.getOriginalIndividuals(abstractObject));

			for (Integer originalSubject : originalSubjects) {
				for (Integer originalObject : originalObjects) {

					IndividualType oldTypeOfSubject = this.typeComputor.computeType(originalSubject);
					Set<Integer> oldSetOfSubjectIndividuals = this.mapType2Individuals.get(oldTypeOfSubject);

					IndividualType oldTypeOfObject = this.typeComputor.computeType(originalObject);
					...
					if (this.orarOntology.addRoleAssertion(originalSubject, role, originalObject)) {
						...
						// }
//						this.individualsHavingNewTypes.add(originalSubject);
//						this.individualsHavingNewTypes.add(originalObject);
						/*
						 * remove individuals from (old) map: type-->individuals
						 */
						removeIndividuslFromOldMapType2Individuals(oldTypeOfSubject,oldSetOfSubjectIndividuals,originalSubject);
						removeIndividuslFromOldMapType2Individuals(oldTypeOfObject,odlSetOfObjectIndividuals,originalObject);
						
						this.typeComputor.computeTypeIncrementally(originalSubject);
						this.typeComputor.computeTypeIncrementally(originalObject);
	...
	private void transferRoleAssertionBetweenUAndNominals() {
		...
		for (int index = 0; index < size; index++) {
			...
			Set<Integer> originalSubjects = this.dataForTransferingEntailments.getMap_UAbstractIndiv_2_OriginalIndivs()
					.get(abstractSubject);
			// Set<Integer> originalSubjects =
			// this.dataForTransferingEntailments.getOriginalIndividuals(abstractSubject);
			if (originalSubjects == null) {
				originalSubjects = new HashSet<Integer>();
			} else originalSubjects= new HashSet<>(originalSubjects);
			for (Integer originalSubject : originalSubjects) {

				IndividualType oldTypeOfSubject = this.typeComputor.computeType(originalSubject);
				Set<Integer> odlSetOfSubjectIndividuals = this.mapType2Individuals.get(oldTypeOfSubject);

				IndividualType oldTypeOfObject = this.typeComputor.computeType(nominalInteger);
				Set<Integer> odlSetOfObjectIndividuals = this.mapType2Individuals.get(oldTypeOfObject);
				...
				if (this.orarOntology.addRoleAssertion(originalSubject, role, nominalInteger)) {
					...
					// }
//					this.individualsHavingNewTypes.add(originalSubject);
//					this.individualsHavingNewTypes.add(nominalInteger);
					/*
					 * remove individuals from (old) map: type-->individuals
					 */
					removeIndividuslFromOldMapType2Individuals(oldTypeOfSubject,odlSetOfSubjectIndividuals,originalSubject);
					removeIndividuslFromOldMapType2Individuals(oldTypeOfObject,odlSetOfObjectIndividuals,nominalInteger);

					this.typeComputor.computeTypeIncrementally(originalSubject);
					this.typeComputor.computeTypeIncrementally(nominalInteger);
					
					this.newRoleAssertions.addRoleAssertion(originalSubject, role, nominalInteger);

					if (this.config.getDebuglevels().contains(DebugLevel.TRANSFER_ROLEASSERTION)) {
						logger.info("***DEBUG***TRANSFER_ROLEASSERTION:");
						logger.info(IndividualIndexer.getInstance().getIndividualString(originalSubject) + ", " + role
								+ ", " + nominal);
						logger.info("updated=true");
					}
				}
	...
	private void transferRoleAssertionBetweenNominalAndU() {
		...
		for (int index = 0; index < size; index++) {
			...
			for (Integer eachOriginalObject : originalObjects) {
				/*
				 * prepare for removing individuals from map: (old) type-->
				 * individuals
				 */
				IndividualType oldTypeOfSubject = this.typeComputor.computeType(nominalInteger);
				Set<Integer> odlSetOfSubjectIndividuals = this.mapType2Individuals.get(oldTypeOfSubject);
				...
				if (this.orarOntology.addRoleAssertion(nominalInteger, role, eachOriginalObject)) {
					...
					// }
//					this.individualsHavingNewTypes.add(nominalInteger);
//					this.individualsHavingNewTypes.add(eachOriginalObject);
					/*
					 * remove individuals from (old) map: type-->individuals
					 */
					removeIndividuslFromOldMapType2Individuals(oldTypeOfSubject,odlSetOfSubjectIndividuals,nominalInteger);
					removeIndividuslFromOldMapType2Individuals(oldTypeOfObject,odlSetOfObjectIndividuals,eachOriginalObject);
					
					this.typeComputor.computeTypeIncrementally(nominalInteger);
					this.typeComputor.computeTypeIncrementally(eachOriginalObject);
