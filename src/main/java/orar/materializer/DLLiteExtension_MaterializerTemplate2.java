...
public abstract class DLLiteExtension_MaterializerTemplate2 implements Materializer {
	// input & output
	...
	// logging
	private static final Logger logger = Logger.getLogger(DLLiteExtension_MaterializerTemplate2.class);
	// shared data
	...
	public DLLiteExtension_MaterializerTemplate2(OrarOntology2 normalizedOrarOntology) {
		// input & output
		...
		this.typeComputor = new BasicTypeComputor(normalizedOrarOntology);
	...
	public void materialize() {
		...
		while (updated) {
			...
			Map<IndividualType, Set<Integer>> typeMap2Individuals = this.typeComputor.computeTypes();
			...
			if (updated) {
				...
				// if (newlyAddedRoleAssertions.getSize()==0 &&
				// newlyAddedSameasAssertions.isEmpty()) {
				// break;
				// }
