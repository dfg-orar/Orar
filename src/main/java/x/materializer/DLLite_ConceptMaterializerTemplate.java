...
import orar.refinement.assertiontransferring.DLLiteR.DLLite_ConceptAssertionTransporter;
...
public abstract class DLLite_ConceptMaterializerTemplate implements Materializer {
	// input & output
	...
	// logging
	private static final Logger logger = Logger.getLogger(DLLite_ConceptMaterializerTemplate.class);
	// shared data
	...
	private long loadingTimeOfInnerReasoner = 0;

	public DLLite_ConceptMaterializerTemplate(OrarOntology2 normalizedOrarOntology) {
		// input & output
	...
	public void materialize() {
		...
		//
		// /*
		// * (2). Compute deductive closure of equality, trans, functionality,
		// and
		// * role subsumsion.
		// */
		...
		if (config.getLogInfos().contains(LogInfo.STATISTIC)) {
			...
			logger.info(
					StatisticVocabulary.CURRENT_LOOP + currentLoop + ";" + StatisticVocabulary.NUMBER_OF_YZ + yandz);
		...
		this.reasoningTimeInSeconds = this.reasoningTimeInSeconds - loadingTimeOfInnerReasoner;
		/*
		 * logging
		 */
	...
	protected AssertionTransporter getAssertionTransporter(
		...
		AssertionTransporter assertionTransporter = new DLLite_ConceptAssertionTransporter(normalizedORAROntology,
